...
3. Connects to a running Unity instance over WebSocket
4. Translates agent-browser commands (snapshot, click, fill, etc.) into Unity
   UI operations, and translates Unity UI state back into agent-browser
   snapshot format

On the Unity side, a C# MonoBehaviour runs a WebSocket server that:
...
agent-browser CLI (unmodified, production binary)
  ...
  v
ABU Daemon (our TypeScript project)
  |  WebSocket, JSON
  v
Unity / Dreamtides (C# MonoBehaviour server)
```

## Key design questions to investigate

- What is the minimal subset of agent-browser commands we need to implement?
  The full protocol has ~100 commands; a game likely only needs snapshot, click,
...
- How do we walk the full Unity scene? The snapshot needs to cover UI Toolkit
  ...
  `Transform` hierarchy). How do we unify these into one coherent tree?
...
- How does the AI know when the UI has settled? After an action, the game may
  ...
  heuristic for detecting when the scene has stabilized (e.g. no VisualElement
  changes for N frames). For cases where the heuristic isn't enough, there
  should be a small, well-specified API that game authors can optionally
  implement (e.g. an `IABUReadyProvider` interface) -- but the goal is to
  minimize the amount of game-specific code required.

- Can we reuse AltTester patterns? AltTester solves similar problems
  (finding Unity objects, simulating input, communicating over WebSocket). We
  should study its approach but build something purpose-fit for AI agents
  rather than test automation.

## Scope of the design document

The design document should cover:

1. The TypeScript daemon: project structure, socket/protocol handling,
   command routing, connection to Unity
2. The Unity C# bridge: WebSocket server, scene walking (UI Toolkit + UGUI +
   3D GameObjects), ref assignment, input simulation, threading model
...
5. A plan for incremental development: what is the smallest thing we can
   demo end-to-end, and how do we build up from there?
6. How to test: can we write automated tests that verify the daemon speaks
   the protocol correctly, independent of Unity?

## Non-goals for the design document

- Modifying agent-browser itself (we use it as-is)
- Game-specific logic in the bridge (it should be general-purpose for any
  Unity game; Dreamtides is just the testbed)
