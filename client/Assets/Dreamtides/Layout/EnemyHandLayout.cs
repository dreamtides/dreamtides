...
namespace Dreamtides.Layout
{
  public class EnemyHandLayout : LandscapeHandLayout
  {
    public override Vector3? CalculateObjectRotation(int index, int count)
    {
      ...
      {
        return new Vector3(x: rotation.x, y: rotation.y, z: rotation.z + 180f);
      }
      else
      {
        return null;
    ...
    public override Vector3 CalculateObjectPosition(int index, int count)
    {
      ...
      if (
        index < Objects.Count
        && Objects[index] is Card card
        && card.NullableCardView?.Revealed != null
      )
      {
        ...
        return new Vector3(x: basePosition.x, y: basePosition.y + 0.25f, z: basePosition.z - 0.25f);
      }
      else
      {
        return basePosition;
