...
namespace Dreamtides.Utils
{
  ...
  {
    public static T Instantiate<T>(T prefabComponent, Vector3? position = null)
      where T : Component =>
      InstantiateGameObject<T>(Errors.CheckNotNull(prefabComponent).gameObject, position);

    public static T InstantiateGameObject<T>(GameObject prefab, Vector3? position = null)
      where T : Component
    {
      Errors.CheckNotNull(prefab);
      var instantiated = UnityEngine.Object.Instantiate(
        prefab,
        position ?? 1000f * Vector3.one,
        Quaternion.identity
      );
    ...
    public static T Get<T>(Component component)
      where T : Component
    {
      ...
      where T : Component
    {
      ...
      {
        throw new NullReferenceException(
          $"Expected a component of type {typeof(T).FullName} on {gameObject.name}"
        );
      }
