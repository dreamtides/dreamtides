...
namespace Dreamtides.Tests
{
  public class TurnSequenceTests : IntegrationTest
  {
    ...
    public IEnumerator TestEndTurnOpponentScoresPoints()
    {
      ...
      Assert.That(
        Registry.Layout.EnemyStatusDisplay.TotalSpark._text.text.Contains("10"),
        Is.True,
        "Enemy spark should be 10"
      );
      yield return TestClickInputProvider.ClickOn(Registry, Registry.Layout.PrimaryActionButton);
      Assert.That(
        Registry.TestHelperService.DidObjectMove(Registry.Layout.UserStatusDisplay.TotalSpark),
        Is.True,
        "User spark should have moved"
      );
      Assert.That(
        Registry.TestHelperService.DidObjectMove(Registry.Layout.EnemyStatusDisplay.TotalSpark),
        Is.True,
        "Enemy spark should have moved"
      );
      Assert.That(
        Registry.Layout.EnemyStatusDisplay.Score._text.text.Contains("10"),
        Is.True,
