...
namespace Dreamtides.Tests
{
  public class BasicActionTests : IntegrationTest
  {
    ...
    public IEnumerator TestPlayCharacter()
    {
      ...
      yield return TestDragInputProvider.DragTo(Registry, card, Registry.Layout.DefaultStack);
    ...
    public IEnumerator TestSelectTargetDissolveEnemy()
    {
      yield return Connect();
      yield return PerformAction(
        TestBattle.New().RemovePlayerHands().AddCardsToBattlefield(DisplayPlayer.Enemy, 8).Build()
      );

      yield return PerformAddCardAction(
        TestBattle.New().AddCardToHand(DisplayPlayer.User, TestCards.TestDissolve).Build()
      );
      ...
      foreach (var enemy in Registry.Layout.EnemyBattlefield.Objects)
      {
        ...
        Assert.That(
          enemyCard._battlefieldOutline.color,
    ...
    public IEnumerator TestSelectCardOnStackPrevent(
      [ValueSource("MobileAndDesktop")] GameViewResolution resolution
    )
    {
      ...
      yield return PerformAddCardAction(
        TestBattle.New().AddCardToHand(DisplayPlayer.User, TestCards.TestCounterspell).Build()
      );
      ...
      yield return PerformAddCardAction(
        TestBattle.New().AddCardToHand(DisplayPlayer.Enemy, TestCards.TestDrawOne).Build()
      );
      ...
      yield return PerformOpponentAction(
        new BattleAction
        {
          BattleActionClass = new BattleActionClass { PlayCardFromHand = long.Parse(enemyCard1) },
        }
      );
      ...
      yield return PerformOpponentAction(
        new BattleAction
        {
          BattleActionClass = new BattleActionClass { PlayCardFromHand = long.Parse(enemyCard2) },
        }
      );
      ...
      AssertArrowBetween(
        userPrevent1,
        Registry.CardService.GetCard(enemyCard1),
        "userPrevent1 should target enemyCard1"
      );

      yield return TestClickInputProvider.ClickOn(
        Registry,
        Registry.CardService.GetCard(enemyCard2)
      );
    ...
    {
      Assert.That(
        Registry.ArrowService._arrows.Count,
        Is.EqualTo(1),
        "One arrow should be visible"
      );
      var arrow = Registry.ArrowService._arrows[0];
      Assert.That(
        arrow.Source.gameObject,
