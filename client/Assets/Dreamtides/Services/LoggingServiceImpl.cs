...
namespace Dreamtides.Services
{
  ...
  public class LoggingServiceImpl : LoggingService
  {
    ...
    public override void EndSpan(LogSpanName name)
    {
      ...
      {
        var request = new ClientLogRequest { Entry = rootSpanEntry };
    ...
    private LogEntry BuildNestedSpanStructure(List<ActiveSpan> spans)
    {
      if (spans.Count == 1)
      {
        return new LogEntry
        {
          EventSpan = new EventSpan { Name = spans[0].Name, Entries = spans[0].Entries },
        };
      }
      ...
      return new LogEntry
      {
        EventSpan = new EventSpan { Name = rootSpan.Name, Entries = rootSpan.Entries },
      };
    }
    ...
    private void FlushSpanStack()
    {
      ...
      var request = new ClientLogRequest { Entry = logEntry };
    ...
    )
    {
      ...
      {
        Event = new Schema.Event { LogType = logType, Message = formattedMessage },
      };
    ...
    private static string FormatMessageWithArguments(
      string message,
      Dictionary<string, string> arguments,
      string? source = null
    )
    ...
    private void OnUnityLogMessageReceived(
      string condition,
      string _stackTrace,
