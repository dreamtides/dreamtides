    ...
    static void ApplyNode(Registry registry, FlexNode node, VisualElement element)
    {
      ...
      if (element is INodeCallbacks callbacks)
      {
        if (node.HoverStyle != null)
        {
          ...
          hoverStyle = MasonUtils.MergeStyles(hoverStyle, node.Style);
          hoverStyle = MasonUtils.MergeStyles(hoverStyle, node.OnAttachStyle);
          hoverStyle = MasonUtils.MergeStyles(hoverStyle, node.HoverStyle);

          callbacks.SetCallback(Callbacks.Event.MouseEnter, () =>
          {
            if (node.EventHandlers?.OnMouseEnter != null)
            {
              registry.ActionService.PerformAction(MasonUtils.ToUserAction(node.EventHandlers.OnMouseEnter));
            }
          ...
          callbacks.SetCallback(Callbacks.Event.MouseLeave, () =>
          {
            ...
            {
              registry.ActionService.PerformAction(MasonUtils.ToUserAction(node.EventHandlers.OnMouseLeave));
            }

            var originalStyle = new FlexStyle();
            originalStyle = MasonUtils.MergeStyles(originalStyle, node.Style);
            originalStyle = MasonUtils.MergeStyles(originalStyle, node.OnAttachStyle);
        ...
        else
        {
          SetCallback(registry, callbacks,
            MasonUtils.ToUserAction(node.EventHandlers?.OnMouseEnter),
            Callbacks.Event.MouseEnter);
          SetCallback(registry, callbacks,
            MasonUtils.ToUserAction(node.EventHandlers?.OnMouseLeave),
        ...
        if (node.PressedStyle != null)
        {
          ...
          pressedStyle = MasonUtils.MergeStyles(pressedStyle, node.Style);
          pressedStyle = MasonUtils.MergeStyles(pressedStyle, node.OnAttachStyle);
          pressedStyle = MasonUtils.MergeStyles(pressedStyle, node.PressedStyle);
          ...
          callbacks.SetCallback(Callbacks.Event.MouseDown, () =>
          {
            ...
            {
              registry.ActionService.PerformAction(MasonUtils.ToUserAction(onMouseDown));
            }
          });
          callbacks.SetCallback(Callbacks.Event.MouseUp, () =>
          {
            var style = new FlexStyle();
            style = MasonUtils.MergeStyles(style, node.Style);
            style = MasonUtils.MergeStyles(style, node.OnAttachStyle);
            ...
            if (node.HoverStyle != null)
            {
              ...
              style = MasonUtils.MergeStyles(style, node.Style);
              style = MasonUtils.MergeStyles(style, node.OnAttachStyle);
              style = MasonUtils.MergeStyles(style, node.HoverStyle);
            }
            ...
            {
              registry.ActionService.PerformAction(MasonUtils.ToUserAction(onMouseUp));
            }
          });
        }
        else
        {
          SetCallback(registry, callbacks,
            MasonUtils.ToUserAction(node.EventHandlers?.OnMouseDown),
            Callbacks.Event.MouseDown);
          SetCallback(registry, callbacks,
            MasonUtils.ToUserAction(node.EventHandlers?.OnMouseUp),
        ...
        if (node.OnAttachStyle != null)
        {
          ...
          attachStyle = MasonUtils.MergeStyles(attachStyle, node.Style);
          attachStyle = MasonUtils.MergeStyles(attachStyle, node.OnAttachStyle);
        ...
        SetCallback(registry, callbacks,
          MasonUtils.ToUserAction(node.EventHandlers?.OnClick), Callbacks.Event.Click);
        SetCallback(registry, callbacks,
          MasonUtils.ToUserAction(node.EventHandlers?.OnLongPress), Callbacks.Event.LongPress);
        SetCallback(registry, callbacks,
          MasonUtils.ToUserAction(node.EventHandlers?.OnFieldChanged), Callbacks.Event.FieldChanged);
