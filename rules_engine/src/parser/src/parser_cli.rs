...
#[command(
  ...
  parser parse-file --input tabula/cards.toml --output parsed_cards.json

  # Parse abilities from all TOML files in a directory
  parser parse-abilities --directory tabula --output parsed_abilities.json

  # Verify a parsed abilities JSON file matches the TOML source
  parser verify-abilities --directory tabula --input parsed_abilities.json

  # Verify all cards can be lexed and variables resolved
  parser verify tabula/cards.toml

  # Parse event with reclaim ability
  parser parse \"Draw {cards}. Discard {discards}.\\n\\n{ReclaimForCost}\" \\
    --vars \"cards: 2, discards: 2, reclaim: 2\" --stage lex --format json

  # Parse triggered ability with cost
  parser parse \"Abandon {count-allies}: {Reclaim} this character.\" \\
    --vars \"allies: 3\" --stage resolve-variables

  # Parse static ability
  parser parse \"Once per turn, you may play a character with cost {e} or less from your void.\" \\
    --vars \"e: 2\" --stage lex"
)]
struct Cli {
    #[command(subcommand)]
