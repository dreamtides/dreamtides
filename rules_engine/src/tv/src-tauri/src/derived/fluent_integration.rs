...
/// Transforms numbered message references like `{e1}` into `{e(e: $e1)}`.
///
/// This handles the convention where variables like `e1`, `cards2` should call
/// the base message (`e`, `cards`) with the numbered variable as the argument.
fn expand_numbered_message_refs(expression: &str) -> String {
    let mut result = String::with_capacity(expression.len() * 2);
    let mut chars = expression.chars().peekable();
    let mut depth = 0;
        ...
        match ch {
            '{' => {
                ...
                if depth == 1 {
                    ...
                    if next_char == Some('}') && !name.is_empty() {
                        if let Some((base, suffix)) = split_numbered_suffix(&name) {
                            ...
                            depth -= 1;
                            tracing::debug!(
                                component = "derived.fluent",
                                original = name,
                                base = base,
                                suffix = suffix,
                                "Expanded numbered message reference"
                            );
                            continue;
                        }
                    }
...
/// Splits a name into base and numeric suffix if it ends with digits.
///
/// Examples: "e1" -> Some(("e", "1")), "cards2" -> Some(("cards", "2")), "mode" -> None
fn split_numbered_suffix(name: &str) -> Option<(&str, &str)> {
    ...
    if split_pos > 0 && split_pos < name.len() {
...
fn build_temp_message(expression: &str) -> String {
    ...
    let expanded = expand_numbered_message_refs(&expanded);
