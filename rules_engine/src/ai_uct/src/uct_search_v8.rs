...
/// Monte Carlo search algorithm (V8: no positioning filter).
///
/// Identical to V5 except `should_override_positioning` is disabled —
/// BeginPositioning is never filtered from the root actions. Used as a
/// control to measure whether positioning filtering affects win rate.
pub fn search(
    ...
    debug!(?total_iterations, ?action, ?num_threads, "Picked AI action (V8)");
...
/// No-op positioning filter: never restricts BeginPositioning.
fn should_override_positioning(_battle: &BattleState, _player: PlayerName) -> bool {
    false
}
