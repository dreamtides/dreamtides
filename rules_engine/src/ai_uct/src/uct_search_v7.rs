...
/// Monte Carlo search algorithm (V7: smart positioning heuristic).
///
/// Identical to V5 except the root-level `should_override_positioning`
/// uses combat-aware logic: conservative by default (keep characters in
/// back rank), but allows positioning for chump blocking, alpha strikes,
/// near-win scenarios, and dominant-spark attacks.
pub fn search(
    ...
    debug!(?total_iterations, ?action, ?num_threads, "Picked AI action (V7)");
...
/// Smart positioning heuristic based on combat math.
///
/// Conservative by default: characters stay in the back rank unless
/// there is a clear tactical reason to move forward.
///
/// Allows positioning when:
/// 1. Opponent has unblocked front-rank attackers that must be blocked to
///    prevent scoring (chump block).
/// 2. The AI can score enough unblocked points to win the game even with
///    optimal opponent blocking (alpha strike).
/// 3. Scoring this turn would leave the AI within one unblocked hit of winning
///    (near-win aggression).
/// 4. The AI has a back-rank character with higher spark than every opponent
///    character, guaranteeing favorable trades.
///
/// Returns true to filter out BeginPositioning (stay conservative).
fn should_override_positioning(battle: &BattleState, player: PlayerName) -> bool {
    ...
    let ai_points = battle.players.player(player).points;
    let points_to_win = battle.rules_config.points_to_win;
    let opp_front = &battle.cards.battlefield(opponent).front;
    let ai_front = &battle.cards.battlefield(player).front;

    // 1. Chump block: opponent has unblocked front-rank attackers that will score
    //    on their next Judgment. Must position blockers.
    let unblocked_opp_damage: u32 = (0..front_size)
        .filter(|&col| ai_front[col].is_none())
        .filter_map(|col| opp_front[col])
        .filter_map(|id| card_properties::spark(battle, opponent, id))
        .map(|s| s.0)
        .sum();

    if unblocked_opp_damage > 0 {
        return false;
    }

    // Gather AI back-rank character spark values.
    let ai_back_sparks: Vec<u32> = battle.cards.battlefield(player).back[..back_size]
        .iter()
    ...
    if ai_back_sparks.is_empty() {
        return true;
    }

    let ai_best_back = *ai_back_sparks.iter().max().unwrap();

    // All opponent battlefield characters are potential blockers after
    // the opponent repositions on their turn.
    let opp_sparks: Vec<u32> = battle
        .cards
        ...
        .map(|s| s.0)
        .collect();
    let opp_char_count = opp_sparks.len();
    let opp_best_spark = opp_sparks.iter().copied().max().unwrap_or(0);

    // Build the best possible attacking force: current AI front-rank
    // characters plus all back-rank characters, capped at front_size.
    // Sort descending so the highest-spark attackers come first.
    let mut all_attackers: Vec<u32> = (0..front_size)
        .filter_map(|col| ai_front[col])
        .filter_map(|id| card_properties::spark(battle, player, id))
        .map(|s| s.0)
        .chain(ai_back_sparks.iter().copied())
        .collect();
    all_attackers.sort_unstable_by(|a, b| b.cmp(a));
    all_attackers.truncate(front_size);

    // Optimal opponent blocking: the opponent places bodies in front of
    // the highest-spark AI attackers first (any blocker prevents scoring
    // regardless of spark comparison). Unblocked attackers are the
    // lowest-spark ones that the opponent couldn't cover.
    let num_blocked = cmp::min(opp_char_count, all_attackers.len());
    let unblocked_score: u32 = all_attackers[num_blocked..].iter().sum();

    // 2. Alpha strike: unblocked scoring wins the game outright.
    if ai_points.0 + unblocked_score >= points_to_win.0 {
        return false;
    }

    // 3. Near-win: scoring this turn leaves us within one unblocked hit of winning
    //    on the following turn.
    let remaining_after = points_to_win.0.saturating_sub(ai_points.0 + unblocked_score);
    if unblocked_score > 0 && remaining_after <= ai_best_back {
        return false;
    }

    // 4. Dominant character: AI has a back-rank character with higher spark than
    //    every opponent character. It will win any 1v1 trade and survive on the
    //    front rank, depleting the opponent's board.
    if ai_best_back > opp_best_spark {
        return false;
    }

    // Default: conservative, stay in back rank.
    true
}
