...
pub fn run_start(
    ...
    tracing::info!(
        operation = "worker_start", worker = % worker_name, worktree = % worktree_path
        .display(), prompt_length = full_prompt.len(), prompt_cmd = ? prompt_cmd,
        self_review, "Starting worker with prompt"
    );
    tracing::debug!(
        operation = "worker_start_prompt", worker = % worker_name, full_prompt = %
...
fn copy_serena_config(config: &Config, worktree_path: &Path) -> Result<()> {
    ...
    tracing::debug!(
        operation = "copy_serena_config", source = % source_serena.display(), dest = %
        dest_serena.display(), "Copied Serena config to worktree"
    );
    Ok(())
}
