...
use crate::{config, git, recovery, sound, transcript_logger, transcript_reader};
...
pub fn handle_stop(
    ...
    {
        tracing::debug!(
            worker = % worker_name, old_path = ? w.transcript_path, new_path = % path,
    ...
    if is_overseer_worker(worker_name) {
        tracing::debug!(
            worker = % worker_name, "Stop hook from overseer session, ignoring"
        );
        return Ok(());
    }
    ...
    match current_status {
        WorkerStatus::Working | WorkerStatus::Rejected => {
            ...
            tracing::info!(
                worker = % worker_name, commit_sha = % commit_sha, commit_msg = %
                first_line, "Stop hook: transitioning worker to NeedsReview"
            );
            let _ = transcript_logger::archive_transcript(
                worker_name,
                transcript_file.as_deref(),
        ...
        WorkerStatus::Reviewing => {
            let current_sha = match git::get_head_commit(&worktree_path) {
                ...
                Err(e) => {
                    tracing::error!(
                        worker = % worker_name, error = % e, worktree = % worktree_path
                        .display(),
            ...
            if sha_changed {
                tracing::info!(
                    worker = % worker_name, old_sha = ? stored_sha, new_sha = %
                    current_sha,
        ...
        _ => {
            tracing::debug!(
                worker = % worker_name, status = ? current_status,
...
impl Patrol {
        ...
        for worker_name in worker_names {
            ...
            if worker_status == WorkerStatus::Idle {
                ...
                match git::is_worktree_clean(worktree_path) {
                    Ok(false) => {
                        tracing::error!(
                            worker = % worker_name, worktree = % worktree_path.display(),
                    ...
                    Err(e) => {
                        tracing::error!(
                            worker = % worker_name, error = % e, worktree = %
                            worktree_path.display(),
                ...
                match git::has_commits_ahead_of(worktree_path, origin_branch) {
                    Ok(true) => {
                        ...
                        if commits_first_detected.is_none() {
                            tracing::info!(
                                worker = % worker_name, status = ? worker_status,
                            ...
                            if elapsed_since_commits >= FALLBACK_DELAY_SECS {
                                ...
                                tracing::info!(
                                    ...
                                    elapsed_since_commits, if hook_config_missing {
                                    "Hook config is MISSING - this is likely the cause" } else {
                                ...
                                if !hook_config_exists {
                                    tracing::error!(
                                        worker = % worker_name, elapsed_since_commits_secs =
                                        elapsed_since_commits, remaining_secs = FALLBACK_DELAY_SECS
                                        - elapsed_since_commits, hook_config_path = %
    ...
    fn send_pending_rebase_prompts(&self, state: &mut State) -> Result<()> {
        ...
        for worker_name in worker_names {
            ...
            let session_id = worker.session_id.clone();
            tracing::info!(
                worker = % worker_name, conflicts = ? conflicts,
            ...
            if let Err(e) = sender.send(&session_id, &conflict_prompt) {
                ...
                tracing::info!(
                    worker = % worker_name, "Rebase conflict prompt sent successfully"
                );
                if let Some(w) = state.get_worker_mut(&worker_name) {
                    w.pending_rebase_prompt = false;
                }
    ...
    fn retry_stale_clear_commands(&self, state: &mut State) -> Result<()> {
        ...
        for worker_name in worker_names {
            ...
            if worker.status != WorkerStatus::Idle {
                tracing::debug!(
                    worker = % worker_name, status = ? worker.status, elapsed_secs =
                    elapsed, "Worker has pending prompt but is not Idle, skipping retry"
                );
                continue;
            }
            ...
            if retry_count >= MAX_CLEAR_RETRIES {
                tracing::warn!(
                    worker = % worker_name, retry_count = retry_count, max_retries =
                    MAX_CLEAR_RETRIES,
                    "/clear retry limit exceeded - attempting full session restart as fallback. \
                     This typically happens when Claude becomes unresponsive after worktree \
                     recreation during accept."
                );
                let worktree_path = PathBuf::from(&worker.worktree_path);
                if let Err(e) =
                    restart_unresponsive_session(&worker_name, &session_id, &worktree_path, state)
                {
                    tracing::error!(
                        worker = % worker_name, error = % e,
                        "Failed to restart unresponsive session. Shutting down for remediation."
                    );
                    anyhow::bail!(
                        "Session restart failed for worker '{}' after {} /clear attempts: {}",
                        worker_name,
                        retry_count,
                        e
                    );
                ...
                tracing::info!(
                    worker = % worker_name,
                    "Session restarted successfully, waiting for SessionStart hook"
                );
                let state_path = state::get_state_path();
                if let Err(e) = state.save(&state_path) {
                    tracing::error!(
                        "Failed to save state after session restart for '{}': {}",
                        worker_name,
                        e
                    );
                }
            ...
            tracing::info!(
                worker = % worker_name, elapsed_secs = elapsed, threshold_secs =
                STALE_THRESHOLD_SECS, retry_count = retry_count + 1, max_retries =
                MAX_CLEAR_RETRIES,
            ...
            if let Err(e) = sender.send(&session_id, "/clear") {
                tracing::error!(
                    worker = % worker_name, error = % e, "Failed to resend /clear"
                );
                continue;
            }
            if let Some(w) = state.get_worker_mut(&worker_name) {
    ...
    fn detect_offline_transition(&self, worker_name: &str, session_id: &str) -> WorkerTransition {
        if session::session_exists(session_id) {
            tracing::info!(
                worker = % worker_name, session_id = % session_id,
    ...
    fn rebase_pending_reviews(
        ...
        for worker_name in workers_to_check {
            ...
            if !has_commits_ahead {
                ...
                tracing::info!(
                    worker = % worker_name, worker_head = % worker_head, origin_ref = %
                    origin_branch_sha, expected_commit_sha = % expected_sha, heads_match
                    = (worker_head == origin_branch_sha),
            ...
            tracing::debug!(
                worker = % worker_name, worker_head = % worker_head, origin_ref = %
                origin_branch_sha, merge_base = % merge_base,
            ...
            tracing::info!(
                worker = % worker_name, merge_base = % merge_base, origin_ref = %
                origin_branch_sha,
...
fn handle_session_start(
    ...
    if worker.status == WorkerStatus::Offline {
        ...
        tracing::info!(
            worker = % worker_name, claude_session_id = % session_id, tmux_session = %
            tmux_session, prompt_len = pending_prompt.len(), prompt_cmd = ?
            pending_prompt_cmd, transcript_path = ? transcript_path,
        ...
        if let Err(e) = tmux_sender.send(&tmux_session, &pending_prompt) {
            tracing::error!(
                worker = % worker_name, tmux_session = % tmux_session, error = % e,
        ...
        tracing::info!(
            worker = % worker_name, session_id = % session_id, current_status = ? worker
            .status, has_prompt = ! worker.current_prompt.is_empty(),
...
fn handle_session_end(
    ...
    if is_overseer_worker(worker_name) {
        tracing::debug!(
            worker = % worker_name, reason = % reason,
    ...
    if reason == "clear" {
        tracing::debug!(
            worker = % worker_name, current_status = ? worker.status,
    ...
    let api_error_info = transcript_path.and_then(|path| {
        ...
        match transcript_reader::scan_transcript_for_api_errors(path, 50) {
            Ok(info) => {
                if info.has_errors() {
                    tracing::info!(
                        worker = % worker_name, error_count = info.error_count,
                        has_500_error = info.has_500_error, has_rate_limit = info
                        .has_rate_limit_error, last_error = ? info
                        .last_error_message, "Detected API errors in transcript"
                    );
                }
                Some(info)
            }
            Err(e) => {
                tracing::debug!(
                    worker = % worker_name, error = % e, transcript_path = % path
                    .display(), "Failed to scan transcript for API errors"
                );
                None
            }
        }
    ...
    let is_api_error = api_error_info
        ...
        .unwrap_or(false);
    tracing::info!(
        worker = % worker_name, reason = % reason, is_crash = is_crash, is_api_error =
        is_api_error, "Worker offline via SessionEnd hook"
    );
    if let Some(w) = state.get_worker_mut(worker_name) {
        ...
        if is_api_error {
            ...
            tracing::warn!(
                worker = % worker_name, api_error_count = w.api_error_count, reason = %
                reason,
            ...
            tracing::error!(
                worker = % worker_name, crash_count = w.crash_count, reason = % reason,
...
/// Restarts an unresponsive TMUX session by killing and recreating it.
///
/// This is a fallback mechanism when `/clear` commands fail to restart Claude
/// Code. This can happen when the worktree is removed and recreated during
/// accept, leaving Claude in an unresponsive state because its working
/// directory was deleted.
fn restart_unresponsive_session(
    worker_name: &str,
    session_id: &str,
    worktree_path: &Path,
    state: &mut State,
    ...
    tracing::info!(
        worker = % worker_name, session_id = % session_id, worktree = % worktree_path
        .display(),
        "Attempting to restart unresponsive session by killing and recreating it"
    );
    if session::session_exists(session_id) {
        tracing::info!(worker = % worker_name, "Killing unresponsive TMUX session");
        if let Err(e) = session::kill_session(session_id) {
            tracing::warn!(
                worker = % worker_name, error = % e,
                "Failed to kill existing session (may already be dead)"
            );
        }
    ...
    let Some(worker_config) = loaded_config.get_worker(worker_name) else {
        anyhow::bail!("Worker '{}' not found in config.toml during session restart", worker_name);
    };
    tracing::info!(worker = % worker_name, "Starting new TMUX session");
        ...
        && let Err(e) = worker::apply_transition(w, WorkerTransition::ToOffline)
    {
        tracing::warn!(
            worker = % worker_name, error = % e,
            "Failed to transition worker to Offline after session restart"
        );
    }
    tracing::info!(
        worker = % worker_name,
        "Session restart initiated, waiting for SessionStart hook"
    );
    Ok(())
}
