...
fn debug_all_cards(battle: &BattleState) -> DebugAllCards {
    DebugAllCards {
        ...
        p1_void: debug_zone(
            ...
            Zone::Void,
            PlayerName::One,
            battle.cards.void(PlayerName::One).iter().map(CardIdType::card_id),
        ),
        p2_void: debug_zone(
            battle,
            Zone::Void,
            PlayerName::Two,
            battle.cards.void(PlayerName::Two).iter().map(CardIdType::card_id),
        ),
        p1_hand: debug_zone(
            battle,
            Zone::Hand,
            PlayerName::One,
            battle.cards.hand(PlayerName::One).iter().map(CardIdType::card_id),
        ),
        p2_hand: debug_zone(
            battle,
            Zone::Stack,
            PlayerName::Two,
            battle.cards.hand(PlayerName::Two).iter().map(CardIdType::card_id),
        ),
        p1_shuffled_into_deck: debug_zone(
            battle,
            Zone::Deck,
            PlayerName::One,
            battle.cards.shuffled_into_deck(PlayerName::One).iter().map(CardIdType::card_id),
        ),
        p2_shuffled_into_deck: debug_zone(
            battle,
            Zone::Deck,
            PlayerName::Two,
            battle.cards.shuffled_into_deck(PlayerName::Two).iter().map(CardIdType::card_id),
        ),
        ...
        p1_banished: debug_zone(
            ...
            Zone::Banished,
            PlayerName::One,
            battle.cards.banished(PlayerName::One).iter().map(CardIdType::card_id),
        ),
        p2_banished: debug_zone(
            battle,
            Zone::Banished,
            PlayerName::Two,
            battle.cards.banished(PlayerName::Two).iter().map(CardIdType::card_id),
        ),
    }
}
...
fn debug_card_state(
    ...
    DebugCardState {
        ...
        properties: DebugCardProperties {
            ...
            spark: format!(
                "{:?}",
                card_properties::spark(battle, controller, CharacterId(card_id))
            ),
            cost: format!("{:?}", card_properties::converted_energy_cost(battle, card_id)),
...
fn format_prompt_choices(prompt: &PromptType) -> Vec<String> {
    match prompt {
        ...
        PromptType::ModalEffect(prompt) => {
            prompt.choices.iter().map(|choice| format!("{choice:?}")).collect()
        }
