...
pub fn from_hand(
    ...
    let battlefield_full = battle.cards.battlefield(player).character_count()
        >= battle.rules_config.character_limit()
        || battle.cards.battlefield(player).back_row_is_full(battle.rules_config.back_row_size);
