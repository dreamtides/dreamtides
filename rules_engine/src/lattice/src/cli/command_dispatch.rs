...
use crate::cli::commands::doctor_command::doctor_executor;
...
use crate::cli::commands::{
    ...
    completion_command, create_command, dep_command, fmt_command, generate_ids, impact_command,
    label_command, links_from, links_to, mcp_command, mv_command, orphans_command,
...
fn dispatch_command(context: CommandContext, command: Command) -> LatticeResult<()> {
    match command {
        ...
        Command::Label(args) => {
            ...
            label_command::execute(context, args)
        }
        Command::LinksFrom(args) => {
        ...
        Command::Path(args) => {
            ...
            path_command::execute(context, args)
        }
        ...
        Command::Doctor(args) => {
            ...
            doctor_executor::execute(context, args)
        }
        Command::Setup(args) => {
        ...
        Command::Mcp => {
            info!("Dispatching to mcp command");
            mcp_command::execute(context)
        }
    }
}
