...
updated-at: 2026-01-20T07:00:32.253744Z
---
...
2. Add an inline link in MAIN body referencing RELATED: `[RELATED_ID](lattice://RELATED_ID)`
...
When using `--force`, the prune command should convert any inline markdown links that reference the pruned task into plain text.

## Actual Behavior
`lat prune --force` deletes the task but does NOT convert the inline link to plain text. The MAIN task still contains the broken link `[RELATED_ID](lattice://RELATED_ID)` after the prune completes.
