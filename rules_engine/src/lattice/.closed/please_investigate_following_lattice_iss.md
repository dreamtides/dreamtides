...
Please design and implement the --commit flag to `lat create`. This should cause a git commit to be created with the new lattice file after creation completes. The commit message should contain the full text of the lattice document, with the first line formatting like "Create feature request L34567" (or bug report etc). This must ALSO work with `lat create --interactive`, in which case it should create the commit with the full text *after* $EDTOR returns. 
