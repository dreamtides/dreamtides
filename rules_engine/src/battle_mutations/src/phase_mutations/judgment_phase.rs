...
///
/// The active player's front-rank characters are attackers. The non-active
...
pub fn run(battle: &mut BattleState, player: PlayerName, source: EffectSource) -> bool {
    ...
    match (attacker_id, blocker_id) {
        (Some(attacker), Some(blocker)) => {
            battle.turn.judgment_participants.push((player, attacker, position));
            battle.turn.judgment_participants.push((opponent, blocker, position));
            let attacker_spark =
                card_properties::spark(battle, player, attacker).unwrap_or_default();
            let blocker_spark =
                card_properties::spark(battle, opponent, blocker).unwrap_or_default();
        ...
        (Some(attacker), None) => {
            battle.turn.judgment_participants.push((player, attacker, position));
            let spark = card_properties::spark(battle, player, attacker).unwrap_or_default();
