...
pub fn run(battle: &mut BattleData, player: PlayerName, source: Source) {
    let spark = battle_queries::player_battle_queries::spark_total::query(battle, player, source);
    let opponent_spark = battle_queries::player_battle_queries::spark_total::query(
        battle,
        player.opponent(),
        source,
    );
