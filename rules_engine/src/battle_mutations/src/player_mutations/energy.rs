...
pub fn spend(battle: &mut BattleData, player: PlayerName, _source: Source, amount: Energy) {
...
pub fn set(battle: &mut BattleData, player: PlayerName, _source: Source, amount: Energy) {
    let player_state = battle.player_mut(player);
    player_state.current_energy = amount;
}

/// Sets `player`'s produced energy to `amount`.
pub fn set_produced(battle: &mut BattleData, player: PlayerName, _source: Source, amount: Energy) {
    let player_state = battle.player_mut(player);
    player_state.produced_energy = amount;
}

/// Adds `amount` to `player`'s current energy.
pub fn gain(battle: &mut BattleData, player: PlayerName, _source: Source, amount: Energy) {
    let player_state = battle.player_mut(player);
    player_state.current_energy += amount;
}
