use ai_uct::position_assignment;
...
pub fn breakdown(battle: &BattleState, maximizing_player: PlayerName) -> EvaluationBreakdown {
    ...
    EvaluationBreakdown { total: features.iter().map(|f| f.value).sum(), features }
}
...
fn side_board_presence_score(battle: &BattleState, player: PlayerName) -> f64 {
    ...
    battle
        ...
        .map(|character_id| {
            ...
            let ready = state.is_some_and(|s| s.played_turn != current_turn)
