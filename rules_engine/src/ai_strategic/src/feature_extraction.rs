...
pub fn extract_state_features(battle: &BattleState, perspective: PlayerName) -> FeatureMap {
    ...
    for feature in evaluation::breakdown(battle, perspective).features {
...
pub fn extract_assignment_features(
    battle: &BattleState,
    perspective: PlayerName,
    ...
    let mut moved_spark = 0.0;
    let mut moved_count = 0.0;
    let opponent = perspective.opponent();

    for &(character_id, placement) in &assignment.placements {
        if let CharacterPlacement::MoveToFrontRank(column) = placement {
            moved_count += 1.0;
            moved_spark += card_properties::spark(battle, perspective, character_id)
                .unwrap_or_default()
                .0 as f64;
            if battle.cards.battlefield(opponent).front[column as usize].is_some() {
                block_count += 1.0;
            } else {
                attack_count += 1.0;
            }
    ...
    features.insert("assignment_attack_count".to_string(), attack_count);
    features.insert("assignment_block_count".to_string(), block_count);
    features.insert("assignment_moved_spark".to_string(), moved_spark);
    features.insert("assignment_moved_count".to_string(), moved_count);
    features
}
