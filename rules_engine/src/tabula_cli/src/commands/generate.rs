...
pub fn generate(output_dir: Option<PathBuf>) -> Result<()> {
    ...
    let string_ids_content = generate_string_ids_string(&tabula_dir)?;
    let output_path = output.join("string_id.rs");
    fs::write(&output_path, string_ids_content).context("Failed to write string_id.rs")?;
    println!("Generated: {}", output_path.display());
...
/// Generates string_id.rs content from strings.ftl.
pub fn generate_string_ids_string(tabula_dir: &Path) -> Result<String> {
