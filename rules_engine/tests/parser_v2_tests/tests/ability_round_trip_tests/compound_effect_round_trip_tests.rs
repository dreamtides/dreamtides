...
fn test_round_trip_judgment_draw_then_discard() {
    ...
    let parsed = parse_ability(original, "cards: 2, discards: 1");
    let serialized = ability_serializer::serialize_ability(&parsed);
    assert_eq!("{Judgment} Draw {cards}. Discard {discards}.", serialized.text);
}
...
fn test_round_trip_materialized_discard_then_draw() {
    ...
    let parsed = parse_ability(original, "discards: 1, cards: 2");
    let serialized = ability_serializer::serialize_ability(&parsed);
    assert_eq!("{Materialized} Discard {discards}. Draw {cards}.", serialized.text);
}
...
fn test_round_trip_abandon_ally_play_character_for_alternate_cost() {
    let original = "Abandon an ally: Play this character for {e}, then abandon it.";
    let parsed = parse_ability(original, "e: 0");
    let serialized = ability_serializer::serialize_ability(&parsed);
    assert_eq!(original, serialized.text);
}
