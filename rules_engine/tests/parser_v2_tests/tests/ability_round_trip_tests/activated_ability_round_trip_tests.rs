...
fn test_round_trip_abandon_an_ally_once_per_turn_reclaim_subtype() {
    ...
    let serialized = ability_serializer::serialize_ability(&parsed);
    assert_eq!("Abandon an ally, once per turn: {Reclaim} {a-subtype}.", serialized.text);
}
...
fn test_round_trip_energy_discard_kindle() {
    ...
    let serialized = ability_serializer::serialize_ability(&parsed);
    assert_eq!("{e}, Discard {discards}: {Kindle}.", serialized.text);
}
...
fn test_round_trip_spend_one_or_more_energy_draw_for_each_energy_spent() {
    ...
    let serialized = ability_serializer::serialize_ability(&parsed);
    assert_eq!("Pay 1 or more {energy-symbol}: Draw {cards} for each {energy-symbol} spent.", serialized.text);
}
