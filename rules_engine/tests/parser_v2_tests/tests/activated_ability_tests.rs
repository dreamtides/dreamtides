...
#[test]
fn test_banish_void_with_min_count_reclaim_this() {
    let result = parse_ability("{Banish} your void with {count} or more cards: {Reclaim} this character.", "count: 5");
    assert_ron_snapshot!(result, @r###"
    Activated(ActivatedAbility(
      costs: [
        BanishYourVoidWithMinCount(5),
      ],
      effect: Effect(ReturnFromYourVoidToPlay(
        target: This,
      )),
