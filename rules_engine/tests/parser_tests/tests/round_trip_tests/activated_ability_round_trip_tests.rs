...
fn test_abandon_allies_count_reclaim_self() {
    assert_rendered_match("Abandon {count_allies($a)}: {Reclaim} this character.", "a: 2");
}
...
fn test_abandon_ally_gain_spark() {
    assert_rendered_match("Abandon an ally: This character gains +{$s} spark.", "s: 1");
}

#[test]
fn test_pay_energy_discard_kindle() {
    assert_rendered_match("{energy($e)}, Discard {cards($d)}: {Kindle($k)}.", "e: 1\nd: 1\nk: 2");
}

#[test]
fn test_abandon_ally_gain_energy() {
    assert_rendered_match("Abandon an ally: Gain {energy($e)}.", "e: 1");
}
...
fn test_abandon_ally_kindle() {
    assert_rendered_match("Abandon an ally: {Kindle($k)}.", "k: 1");
}

#[test]
fn test_abandon_ally_once_per_turn_gain_points() {
    assert_rendered_match("Abandon an ally, once per turn: Gain {points($p)}.", "p: 1");
}
...
fn test_abandon_ally_once_per_turn_reclaim_subtype() {
    assert_rendered_match(
        "Abandon an ally, once per turn: {Reclaim} {@a subtype($t)}.",
        "t: Warrior",
    );
}
...
fn test_abandon_discard_hand_gain_energy() {
    assert_rendered_match("Abandon a character, Discard your hand: Gain {energy($e)}.", "e: 5");
}

#[test]
fn test_pay_energy_draw() {
    assert_rendered_match("{energy($e)}: Draw {cards($c)}.", "e: 3\nc: 1");
}

#[test]
fn test_pay_energy_materialize_copy() {
    assert_rendered_match("{energy($e)}: {Materialize} a copy of an ally.", "e: 4");
}
...
fn test_abandon_ally_gain_energy_equal_cost() {
    assert_rendered_match(
        "Abandon an ally: Gain {energy_symbol} equal to that character's cost.",
        "",
    );
}
...
fn test_abandon_or_discard_dissolve_enemy() {
    assert_rendered_match("Abandon an ally or discard {cards($d)}: {Dissolve} an enemy.", "d: 1");
}

#[test]
fn test_banish_from_hand_play_for_zero_prevent() {
    assert_rendered_match("{Banish} a card from hand: Play this event for {energy($e)}.", "e: 0");
...
fn test_lose_max_energy_play_for_zero_prevent() {
    assert_rendered_match(
        "Lose {maximum_energy($m)}: Play this event for {energy($e)}.",
        "m: 1\ne: 0",
    );
    assert_rendered_match("{Prevent} a played card.", "");
}
...
fn test_fast_abandon_self_prevent_event() {
    assert_rendered_match("{Fast} -- Abandon this character: {Prevent} a played event.", "");
}
...
fn test_pay_variable_energy_draw_per_energy() {
    assert_rendered_match("Pay 1 or more {energy_symbol}: Draw {cards($c)} for each {energy_symbol} spent, then discard {cards($d)}.", "c: 1\nd: 2");
}
