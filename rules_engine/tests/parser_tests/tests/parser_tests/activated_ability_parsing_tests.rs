...
fn test_multi_activated_draw() {
    let result = parse("{ma} {-energy-cost(e: 2)}: Draw {-drawn-cards(n: 1)}.");
    assert_ron_snapshot!(
        result,
        @r###"
    [
...
fn test_abandon_character_with_spark() {
    let result =
        parse("{a} Abandon another character with spark 2 or less: Draw {-drawn-cards(n: 1)}.");
    assert_ron_snapshot!(
        result,
        @r###"
    [
...
fn test_activated_ability_multiple_costs() {
    let result = parse(
        "{ma} {-energy-cost(e: 2)}, Abandon another character with spark 1 or less: Draw {-drawn-cards(n: 2)}.",
    );
    assert_ron_snapshot!(result, @r###"
    [
      Activated(ActivatedAbility(
...
fn test_multiple_costs_abandon_this() {
    let result = parse(
        "{a} {-energy-cost(e: 2)}, Abandon this character, discard your hand: Draw {-drawn-cards(n: 3)}.",
    );
    assert_ron_snapshot!(
        result,
        @r###"
...
fn test_discard_card_draw_gain_point() {
    let result =
        parse("{a} Discard a card: Draw {-drawn-cards(n: 1)}. Gain {-gained-points(n: 1)}.");
    assert_ron_snapshot!(
        result,
        @r###"
    [
