...
def replace_with_symlink(source_dir, dest_base):
    ...
    if source_path.is_symlink():
        try:
            ...
            if current_target == expected_target:
                print(f"Skipping '{source_dir}': already a symlink pointing to correct destination")
                return True
            else:
                print(f"'{source_dir}' is a symlink but points to '{current_target}' instead of '{expected_target}'")
    ...
    if source_path.exists():
        ...
        print(f"Error: Neither source '{source_dir}' nor destination '{dest_path}' exists")
        return False
