...
This design keeps the generated API simple for UI code (no `Result` handling
required), while the interpreter remains testable and explicit about failure
modes. Evaluation errors are programming errors—they indicate the RLF definition
is inconsistent with how it's being used—and should be caught during development.
