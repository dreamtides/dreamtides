...
describe("initializeDraftState", () => {
  it("creates state with only chosen tide + Neutral cards", () => {
    const db = buildTideDB("Bloom", 10);
    const state = initializeDraftState(db, "Bloom");
  ...
  it("excludes cards from other tides", () => {
    const db = buildTideDB("Bloom", 10);
    const state = initializeDraftState(db, "Bloom");
