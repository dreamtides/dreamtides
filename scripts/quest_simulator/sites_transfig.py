...
def _get_design(dc: DeckCard):
    """Extract the CardDesign from a DeckCard."""
...
def _card_name(dc: DeckCard) -> str:
...
def get_applicable_types(design) -> list[TransfigType]:
    """Return all transfiguration types that a CardDesign is eligible for."""
    result: list[TransfigType] = []
    for t in _BASE_TYPES:
        if is_eligible(design, t):
            result.append(t)
    if len(result) >= 2:
...
def _render_transfig_item(
    ...
    gap = max(2, render.CONTENT_WIDTH - vis_line1 - len(right_side))
    line1 = f"{line1}{' ' * gap}{right_side}"

    # Line 2: transfiguration type and effect
    type_label = _colored_type_label(transfig_type)
    note = _TRANSFIG_NOTES[transfig_type]
    line2 = f"    Transfiguration: {type_label}{render.DIM} -- {note}{render.RESET}"

    # Line 3: eligibility reason
    reason = eligibility_explanation(design, transfig_type)
    line3 = f"    {render.DIM}(Eligible: {reason}){render.RESET}"
...
def _render_enhanced_item(
    ...
    # Line 3: eligibility reason
    reason = eligibility_explanation(design, best_type)
    line3 = f"    {render.DIM}(Eligible: {reason}){render.RESET}"
...
def _run_normal(
    ...
    transfig_eligible = [
        ...
        if any(is_eligible(_get_design(dc), t) for t in _BASE_TYPES)
    ]
    ...
    # Build option labels
    option_labels = [f"{t.value} {_card_name(dc)}" for dc, t in candidates]
    ...
    if chosen_index < len(candidates):
        ...
        name = _card_name(dc)
        note = _build_transfig_note(transfig_type, name)
    ...
    if logger is not None:
        logger.log_site_visit(
            ...
            dreamscape=dreamscape_name,
            is_enhanced=is_enhanced,
            choices=[f"{t.value} {_card_name(dc)}" for dc, t in candidates],
...
def _run_enhanced(
    ...
    for dc in eligible_deck_cards:
        design = _get_design(dc)
        applicable = get_applicable_types(design)
    ...
    # Build option labels
    option_labels = [
        f"{assigned_types[i].value} {_card_name(dc)}"
        ...
        if not applicable:
            ...
            if logger is not None:
                logger.log_site_visit(
                    ...
                    dreamscape=dreamscape_name,
                    is_enhanced=is_enhanced,
                    choices=[_card_name(dc) for dc in eligible_deck_cards],
                    choice_made=None,
                    state_changes={},
                )
            return

        note = _build_transfig_note(best_type, name)
        dc.is_transfigured = True
        dc.transfig_note = note
        choice_name = note
        type_label = _colored_type_label(best_type)
