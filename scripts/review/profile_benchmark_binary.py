...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Locate compiled Criterion benchmark binary"
    )
    parser.add_argument(
        "--benchmark",
        required=True,
        help="Benchmark filter (e.g. group/function) passed after -- to cargo criterion",
    )
    parser.add_argument(
        "--package",
        "-p",
        default="battle_benchmarks",
        help="Cargo package name containing the bench target",
    )
    parser.add_argument(
        "--manifest-path",
        default="benchmarks/battle/Cargo.toml",
        help="Path to Cargo.toml for the benchmark package (relative to project root if not absolute)",
    )
    parser.add_argument(
        "--target-dir",
        default=None,
        help="Cargo target directory (defaults to metadata value if omitted; relative paths resolved to project root)",
    )
    parser.add_argument(
        "--samply",
        action="store_true",
        help="If set, invoke 'samply record <binary> <arg>' instead of printing the path",
    )
    ...
    try:
        ...
        if args.target_dir:
            ...
            target_dir_path = Path(
                metadata.get("target_directory", project_root / "target")
            ).resolve()
        ...
        if not picked:
            raise SystemExit(
                f"Could not find built bench binary for target '{bench_target}' in {target_dir_path}/release/deps"
            )
        if args.samply:
            binary_path = str(picked.resolve())
            try:
                completed = subprocess.run(
                    ["samply", "record", binary_path, benchmark_filter]
                )
                return completed.returncode
            except FileNotFoundError:
                raise SystemExit(
                    "'samply' command not found in PATH. Install samply or omit --samply."
                )
        else:
