            ...
            if data.get("reason") == "benchmark-complete":
                ...
                if benchmark_id:
                    results[benchmark_id] = {
                        ...
                        "median_abs_dev": data.get("median_abs_dev", {}).get(
                            "estimate"
                        ),
...
def run_single_benchmark(benchmark_name, extra_args):
    ...
    try:
        ...
        if return_code != 0:
            print_colored(
                f"Error: cargo criterion failed with exit code {return_code}",
                Colors.RED,
            )
            return None
...
def print_run_summary(run_number, results):
    ...
    for bench_id, data in results.items():
        ...
        if estimate:
            print(
                f"{bench_id:<40} {format_time(estimate):<15} "
...
def analyze_variance(all_runs, benchmark_name):
    ...
    for bench_id, times in benchmark_times.items():
        ...
        if ci_95_low and ci_95_high:
            print(
                f"  95% CI:         [{format_time(ci_95_low)}, {format_time(ci_95_high)}]"
            )
        if ci_99_low and ci_99_high:
            print(
                f"  99% CI:         [{format_time(ci_99_low)}, {format_time(ci_99_high)}]"
            )
        ...
        for i, time in enumerate(times, 1):
            ...
            print(
                f"  Run {i:2d}: {format_time(time):>12} ({sign}{deviation:>6.2f}% from mean)"
            )
...
Examples:
  ...
  %(prog)s -n 20 "bench_name" -- --warm-up-time 5
    ...
    print_colored(
        f"Running benchmark '{args.benchmark}' {args.num_runs} times...", Colors.BOLD
    )
    if extra_args:
