"""Configuration system for the draft simulator v2.

Provides a hierarchical configuration with defaults for the archetype-only
draft simulator. Removes RefillConfig, simplifies CardsConfig/RarityConfig,
replaces power weights with rarity weights. Stdlib-only, no external
dependencies.
"""
...
class AgentsConfig:
    ...
    ai_rarity_weight: float = 0.1
    ai_pref_weight: float = 0.7
...
class ScoringConfig:
    ...
    weight_coherence: float = 0.6
    weight_focus: float = 0.4
...
class MetricsConfig:
    ...
    splash_min_tags: int = 2
    exposure_threshold: float = 0.5


@dataclass
class RarityConfig:
    """Rarity tier system parameters (derived from actual TOML data)."""
