#!/usr/bin/env python3
"""Entry point for the draft simulator v2.
...
and demo runs the component demonstrations. Stdlib-only, no external
dependencies.

v2 changes: removed pool_analyzer/refill imports, simplified CLI args,
all cards from rendered-cards.toml, no synthetic generation.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Draft simulator v2 for Dreamtides (archetype-only scoring).",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to rendered-cards.toml (overrides config)",
    )
    parser.add_argument(
        "--original-cards",
        action="store_true",
...
def main() -> None:
    ...
    # Set default rendered_toml_path if not configured
    ...
    # Apply --toml override
    if args.toml:
        cfg.cards.rendered_toml_path = args.toml

    # Apply --original-cards flag
    if args.original_cards:
        cfg.cards.original_only = True
...
def _run_explain() -> None:
    ...
    _explain_section(
        ...
        [
            (
                "rendered_toml_path",
                "Path to rendered-cards.toml containing card definitions.",
            ),
            (
                "archetype_count",
            ...
            (
                "original_only",
                "If true, only load cards with original=true from the TOML.",
            ),
        ],
    )
    ...
    _explain_section(
        ...
        [
            (
                ...
                '(balances preference, signals, and rarity), "greedy" (maximizes '
            ...
            (
                ...
                '(random), "rarity_biased" (favors higher rarity), "curated" '
            ...
            (
                "ai_rarity_weight",
                "Weight of rarity_value tiebreaker in adaptive scoring.",
            ),
            (
                "ai_pref_weight",
    ...
    _explain_section(
        ...
        [
            ...
            (
                "splash_min_tags",
                "Minimum number of archetype tags for an off-plan card "
    ...
    _explain_metric(
        ...
        "off-plan card (multi-tag card with low on-plan fitness). Answers: "
        "after committing, can the drafter still pick up flexible cards?",
        ">= 0.40",
    )
    _explain_metric(
        "Early openness",
    ...
    if describe_pool:
        ...
        print(
            card_generator.describe_card_pool(
                designs, cfg.cards.archetype_count, "rendered-cards.toml"
            )
        )
...
def _run_demo(cfg: SimulatorConfig, seed: int) -> None:
    """Run the component demonstration mode."""
    ...
    strategies = ["uniform", "seeded_themed"]
    strategy_labels = {
        "uniform": "Uniform",
    ...
    _demo_pick_policies(cards, cfg, rng)
    _demo_show_n_strategies(cards, cfg, rng)
    ...
    for card_inst in pack.cards:
        ...
        for i in range(1, min(len(card_inst.design.fitness), archetype_count)):
            if card_inst.design.fitness[i] > top_val:
                ...
                top_arch = i
        tag_count = sum(1 for f in card_inst.design.fitness if f >= 1.0)
        print(
            f"    {colors.card(f'{card_inst.design.name:<30s}')} "
            f"{colors.label('top_arch')}={colors.c(top_arch, 'operator')}  "
            f"{colors.label('tags')}={colors.num(tag_count)}  "
            f"{colors.label('rarity')}={colors.num(f'{card_inst.design.rarity_value:.2f}')}"
        )
...
def _demo_deck_scoring(
    ...
    """Generate a pool, build a preference vector, and print scoring."""
    pool_size = 30
...
def _demo_commitment_detection(
    ...
    for _ in range(pool_size):
        if not available:
            break
        best_arch = argmax(w)
        scored = [(c.fitness[best_arch] + 0.3 * c.rarity_value, c) for c in available]
...
def _demo_pick_policies(
    ...
    for policy_key, policy_label in policies:
        ...
        tag_count = sum(1 for f in pick.design.fitness if f >= 1.0)

        print(
            f"\n  {colors.label(f'{policy_label}:')}"
            f"\n    {colors.label('Selected:')} {colors.card(pick.design.name)}"
            f"  ({colors.label('tags')}={colors.num(tag_count)}, "
            f"{colors.label('rarity')}={colors.num(f'{pick.design.rarity_value:.2f}')}, "
...
def _demo_show_n_strategies(
    ...
    for strategy_key, strategy_label in strategy_labels.items():
        ...
        for card_item in selected:
            ...
            tag_count = sum(1 for f in card_item.design.fitness if f >= 1.0)
            tag = ""
            if strategy_key == "curated":
                if fitness_for_best >= 0.6:
                    tag = f" {colors.ok('[on-plan]')}"
                elif fitness_for_best < 0.3 and card_item.design.rarity_value >= 0.33:
                    tag = f" {colors.c('[off-plan strong]', 'warning')}"
            print(
                f"    {colors.card(f'{card_item.design.name:<30s}')} "
                f"{colors.label('tags')}="
                f"{colors.num(tag_count)}  "
                f"{colors.label('rarity')}="
                f"{colors.num(f'{card_item.design.rarity_value:.2f}')}  "
