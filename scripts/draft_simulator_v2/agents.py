"""Agent state and pick policies for the draft simulator v2.

v2 changes: power term removed from adaptive/signal-ignorant scoring,
replaced with rarity_value term. Tie-breaking uses rarity_value instead
of power. Stdlib-only, no external dependencies.
"""
...
def score_card_adaptive(
    ...
    """Score a card using the Adaptive formula (v2).

    Score = pref_weight * dot(fitness, normalize(w))
          + signal_weight * dot(fitness, openness)
          + rarity_weight * rarity_value.
    """
    pref_weight = agents_cfg.ai_pref_weight
    signal_weight = agents_cfg.ai_signal_weight
    rarity_weight = agents_cfg.ai_rarity_weight
    ...
    rarity_term = rarity_weight * card.design.rarity_value

    return pref_term + signal_term + rarity_term
...
def score_card_signal_ignorant(
    ...
    """Score a card using the Signal-ignorant formula (v2).

    Identical to Adaptive but uses a uniform openness vector.
    """
    pref_weight = agents_cfg.ai_pref_weight
    signal_weight = agents_cfg.ai_signal_weight
    rarity_weight = agents_cfg.ai_rarity_weight
...
def _pick_archetype_loyal(
    ...
    """Pick the card with highest fitness for argmax(w), break ties by rarity_value."""
    from utils import argmax

    best_arch = argmax(agent.w)
    best_card = candidates[0]
    best_fitness = candidates[0].design.fitness[best_arch]
    best_rarity = candidates[0].design.rarity_value

    for card in candidates[1:]:
        fitness = card.design.fitness[best_arch]
        rarity = card.design.rarity_value
        if fitness > best_fitness or (fitness == best_fitness and rarity > best_rarity):
            best_card = card
